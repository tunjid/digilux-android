...
object BrightnessLookup {
    ...
    private const val FLICKER_THRESHOLD = 24
    ...
    fun lookup(query: Int, isByte: Boolean): Int {
        ...
        if (!isByte && query < FLICKER_THRESHOLD) return crawl(query, 0, isByte = false, increasing = true)
        ...
        while (low <= high) {
            ...
            when {
                abs(diff) < 2 -> return crawl(query, mid, isByte, increasing)
                increasing -> low = mid + 1
                else -> high = mid - 1
            }
        }
    ...
    private fun crawl(query: Int, index: Int, isByte: Boolean, increasing: Boolean): Int {
        ...
        if (increasing) while (i < num) if (table[i][key] >= query) return table[i][value] else i++
        else while (i >= 0) if (table[i][key] <= query) return table[i][value] else i--
